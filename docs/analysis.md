...
DualSubs YouTube `v1.5.11` 的数据库默认值为：

```text
Languages = [AUTO, ZH]
```

并在 timedtext 请求中优先读取 `Caches.tlang`。用户曾选择 `zh-Hant` 时，该值会保存到 `@DualSubs.YouTube.Caches.tlang`，优先级高于默认目标语言。

本仓库通过 DualSubs 原生参数 `Languages[0]=AUTO`、`Languages[1]=ZH-HANS` 指定简体目标。历史 `Caches.tlang` 仍需由用户选择一次“中文（简体）”或手动清除。

曾尝试添加独立请求脚本自动改写缓存，但它会与 DualSubs 的 player/timedtext 请求脚本命中相同 URL。在实际 Surge 日志中，独立脚本抢占后只剩 browse 日志，DualSubs 未能添加 `subtype=Translate`，结果仅显示英文原字幕。该方案已在 `zm4` 删除。

## PiP 字幕边界

iOS 系统 PiP 本身具备显示媒体字幕的能力，但前提是内容提供方把字幕作为 PiP/AVPlayer 可消费的字幕轨道交给系统。Maasea 只注入 `pictureInPictureRender` 与后台播放能力，DualSubs 只修改 YouTube API 字幕轨道；二者都不能控制 YouTube 客户端创建 PiP 会话时是否附带字幕渲染层。

Maasea Issue #72 的 PiP 字幕请求已以 `not_planned` 关闭且没有上游修复。因此当前可验证结论是：

- 普通播放器字幕可通过 DualSubs 优化
- PiP 能力可通过 Maasea 解锁
- PiP 内字幕不能由现有 Surge module 保证
- 同时开启 Maasea CaptionLang 与 DualSubs 不是可靠修复，会扩大 protobuf 重编码冲突面
...
7. 选择一次“中文（简体）”后，验证 `@DualSubs.YouTube.Caches.tlang` 为 `zh-Hans`，并确认 player/timedtext/Translate 响应脚本均有日志。
8. 合并版单独测试字幕轨道、双语翻译和播放增强。
9. 单独记录 PiP 是否显示官方字幕与脚本翻译字幕；该项属于客户端能力观察，不作为模块合并阻断条件。
